...
if shared.Future~=nil then print("[Future] Detected future already executed, not executing! Please contant mankacoder on discord if it's not injected.") return end
...
(function() 
    local entity = requesturl("oldvapeEntityHandler.lua", false)
end)()


-- AntiPreloadAsync:
local suc = false
local err = "missing functions"
if hookmetamethod and getnamecallmethod and hookfunction then
    ...
    warn("[Future] AntiPreloadAsync failed to load, missing functions: hookmetamethod getnamecallmethod hookfunction")
end
...
local discordButton = {["Toggle"] = function(...) end} discordButton = OtherWindow.CreateOptionsButton({
    ...
    ["Function"] = function(callback)
        if callback then
            pcall(function() setclipboard("https://discord.gg/CEMMRSvXek") end)
            spawn(function()
				for i = 1, 14 do
					spawn(function()
						local reqbody = {
        ...
        if callback then 
            spawn(function() 
                ...
                if shared.FutureDeveloper then 
                    ...
                    loadstring(game:HttpGet('https://raw.githubusercontent.com/Maanaaaa/Future/main/loadstring.lua', true))()
                end
            end)
        end
    end
...
local ontp = game:GetService("Players").LocalPlayer.OnTeleport:Connect(function(State)
    if State == Enum.TeleportState.Started then
        ...
        if shared.FutureDeveloper then 
            ...
            loadstring(game:HttpGet("https://raw.githubusercontent.com/Maanaaaa/Future/main/Initiate.lua", true))() 
        end
        ]]
...
spawn(function()
    if GuiLibrary["AllowNotifications"] then
        ...
        textlabel.Text = [[<stroke thickness="2">Please join the Future discord server for updates and to leave feedback.  https://discord.gg/CEMMRSvXek</stroke>]]
