...
local State = {
    ...
    FlingCleanup = nil,    -- хук очистки, ставится в БЛОКЕ 7
...
-- «200 local registers» (ловится только компиляцией в Roblox).
...
-- ⚠️ Раньше здесь стоял getSheriffForAutoFarm() — поиск ТОЛЬКО по предмету, без
-- фолбэка на серверные роли. Пока шериф не держал Gun (начало раунда, ган выпал
-- и лежит на карте), функция возвращала nil и выдавала «Sheriff not found» —
-- отсюда «флингается только когда получит ствол». FlingMurderer таким не страдал,
-- потому что зовёт getMurder() с фолбэком на State.PlayerData.
-- Берём getSheriff(): сперва по предмету, затем по роли из серверных данных.
