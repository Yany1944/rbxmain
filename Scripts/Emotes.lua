...
SafeLoad("https://raw.githubusercontent.com/Yany1944/rbxmain/refs/heads/main/Scripts/Notify.lua", "Notify System")
...
local SettingsLib = SafeLoad("https://raw.githubusercontent.com/Yany1944/rbxmain/refs/heads/main/Scripts/Settings.lua", "Settings Library")
...
State.enterCustomAnimationEditor = function(category, animName)
    ...
    getgenv().Notify({ Title = "7yd7 | Animation Editor", Content = "🖱️ Select an animation from the wheel to set for " .. animName, Duration = 3 })
end
...
enterHUDEditor = function()
    ...
    getgenv().Notify({ Title = "7yd7 | HUD Editor", Content = "✏️ Drag elements to reposition", Duration = 3 })
end
...
if UserInputService.KeyboardEnabled then
    -- Open/close the emotes menu with the backtick (`) key.
    -- Bound by KeyCode.Backquote (the physical top-left key), so it works on any keyboard layout.
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if gameProcessed then return end
        if input.KeyCode ~= Enum.KeyCode.Backquote then return end
        if UserInputService:GetFocusedTextBox() then return end

        local ok, emotesWheel = pcall(function()
            return game:GetService("CoreGui").RobloxGui.EmotesMenu.Children.Main.EmotesWheel
        end)
        local isVisible = ok and emotesWheel and emotesWheel.Visible
        pcall(function()
            game:GetService("GuiService"):SetEmotesMenuOpen(not isVisible)
        end)
    end)
